...
class CalculatorData {
    yearlyReduce(today, data, amount, getValue) {
        const dataByYears = data.reduce((acc, item, i) => {
            const year = hooks(today).add(item.period, 'month').year();
            acc[year] = acc[year] || [];
    ...
    yearlyReduce1(today, data, getValue) {
        const dataByYears = data.reduce((acc, item, i) => {
            const date = hooks(today).add(item.period, 'month');
...
class TFSAData extends CalculatorData {
    constructor(today) {
        super();
        this.today = today;
    }
...
class RRSPData extends CalculatorData {
    constructor(today) {
        super();
        this.today = today;
    }
...
class NonRegisteredData extends CalculatorData {
    constructor(today) {
        super();
        this.today = today;
    }
...
class MortgageData extends CalculatorData {
    constructor(today) {
        super();
        this.today = today;
    }
        ...
        if (this.shouldRegenerate(value)) {
            if (value.mortgage && value.mortgageBalance && value.mortgageRate && value.investmentHorizon && value.amount && value.mortgageAmortization) {
                ...
                const dataByYears = amortizationBefore.reduce((acc, itemBefore, i) => {
                    const year = hooks(this.today).add(itemBefore.period, 'month').year();
                    acc[year] = acc[year] || [];
...
const Calculator = class {
    ...
    processGraphData() {
        ...
        if (!this.chartOptions.data.labels || this.chartOptions.data.labels.length !== maxLength) {
            const labels = Array.apply(null, Array(maxLength)).map((_x, i) => hooks(this.today).add(i, 'year').year());
    ...
    static get style() { return ".padding-horizontal{padding:0 10px}.chart-spinner{width:100%;height:100%;position:absolute;display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;-ms-flex-pack:center;justify-content:center;left:0;top:0}.ion-invalid{--border-color:var(--highlight-color-invalid)}.error-item,.ion-color-danger{color:var(--ion-color-danger)}.error-item{--min-height:20px;font-size:80%}"; }
};
